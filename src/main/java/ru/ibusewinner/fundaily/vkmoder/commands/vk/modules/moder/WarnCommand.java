...
public class WarnCommand implements ICommand {
    ...
    public void executeCommand(VkUserItem executor, Message message, List<String> args) {
        try {
            ...
            if (targetUser.getWarnings() <= VKModer.getMaxWarnings()) {
                String answer = "Эй, @id" + targetUser.getId() + " (" + targetUser.getNickname() + ")! Не нарушайте правила беседы!" +
                        "\n" +
                        "\nВы получаете " + targetUser.getWarnings() + " из " + VKModer.getMaxWarnings() + " предупреждение! " +
                ...
                VKModer.getVk().messages().removeChatUser(
                            VKModer.getActor(), message.getPeerId() - 2000000000).userId(message.getReplyMessage().getFromId())
                        .execute();
            }
    ...
    public List<String> getAliases() {
        return List.of("пред","предупреждение","предупредить","warning");
    }
